...
public class ContainerTeleporter extends Container {
	...
	public ContainerTeleporter(InventoryPlayer invPlayer, TileEntityTeleporter tileEntityTeleporter) 
	{
		...
		final int PLAYER_INVENTORY_YPOS = 51+33;
		...
		if (TE_INVENTORY_SLOT_COUNT != tileEntityTeleporter.getSizeInventory()) {
			System.err.println("Mismatched slot count in ContainerTeleporter(" + TE_INVENTORY_SLOT_COUNT
												  + ") and TileTeleporter (" + tileEntityTeleporter.getSizeInventory()+")");
		}
		final int TILE_INVENTORY_XPOS = 76+3;
		final int TILE_INVENTORY_YPOS = 32+3;
		// Add the tile inventory container to the gui
		for (int x = 0; x < TE_INVENTORY_SLOT_COUNT; x++) {
			int slotNumber = x;
			addSlotToContainer(new Slot(tileEntityTeleporter, slotNumber, TILE_INVENTORY_XPOS + SLOT_X_SPACING * x, TILE_INVENTORY_YPOS));
		}
	}
	...
	public void onContainerClosed(EntityPlayer playerIn)
	{
		...
		this.tileEntityTeleporter.closeInventory(playerIn);
	}
}
