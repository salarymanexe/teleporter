...
public class TeleporterNetwork extends WorldSavedData
{
	...
	{
		TeleporterNetwork instance = (TeleporterNetwork)world.getMapStorage().loadData(TeleporterNetwork.class, TeleporterMod.MODID);
		if (instance == null)
		{
	...
	public TeleporterNode getNextNode(EntityLivingBase entity, TeleporterNode sourceNode)
	{
		...
		TileEntityTeleporter sourceTileEntity = (TileEntityTeleporter)entity.worldObj.getTileEntity(sourceNode.pos);
		ItemStack sourceKey = sourceTileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null).getStackInSlot(0);
		...
		for (int i = index + 1; i < this.network.size() + index; ++i)
		{
			...
			if (destinationWorld != null)
			{
				...
				TileEntityTeleporter destinationTileEntity = (TileEntityTeleporter)destinationWorld.getTileEntity(node.pos);
				...
				ItemStack destinationKey = destinationTileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null).getStackInSlot(0);
	...
	{
		return new ChatComponentTranslation("message." + TeleporterMod.MODID + '_' + this.getClass().getSimpleName() + '.' + messageName);
	}
