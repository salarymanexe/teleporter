...
public class TeleporterNetwork extends WorldSavedData {
	...
	private boolean isObstructed(World world, TeleporterNode node)
	{
		...
		/*if ((
				 * bl1 == Blocks.wall_sign || bl1 == Blocks.standing_sign || bl1
				 * == Blocks.lever || bl1 == Blocks.vine || bl1 == Blocks.torch
				 * || bl1 == Blocks.air || bl1 == Blocks.redstone_torch || bl1
				 * == Blocks.ladder
				  bl1.isPassable(world, bl1p))
				&& (
					 * bl2 == Blocks.wall_sign || bl2 == Blocks.standing_sign ||
					 * bl2 == Blocks.lever || bl2 == Blocks.vine || bl2 ==
					 * Blocks.torch || bl2 == Blocks.air || bl2 ==
					 * Blocks.redstone_torch || bl2 == Blocks.ladder
					  bl2.isPassable(world, bl2p))) {
			return false;
		}*/
	}
	...
	public TeleporterNode getNode(BlockPos pos, int dimension, boolean debug) {
		for (int i = 0; i < network.size(); ++i) {
			...
			if (pos.getX() == node.pos.getX() && pos.getY() == node.pos.getY() && pos.getZ() == node.pos.getZ()
				...
				if (debug)
					System.out.println("Getting node at " + pos.getX() + "," + pos.getY() + "," + pos.getZ() + " from network");
				return node;
			}
		}
	...
	public void addNode(TeleporterNode node) 
	{
		...
		network.add(node);
		markDirty();
		System.out.println("Appending node at " + node.pos.getX() + "," + node.pos.getY() + "," + node.pos.getZ() + " to network " + "[" + index + "]");
	}
	...
	public boolean removeNode(BlockPos pos, int dimension) 
	{
		...
		{
			TeleporterNode node = network.get(i);
			if (pos.getX() == node.pos.getX() 
				&& pos.getY() == node.pos.getY() 
				&& pos.getZ() == node.pos.getZ()
				&& dimension == node.dimension) 
			{
				network.remove(node);
				System.out.println("Removing node at "  + pos.getX() + ","  + pos.getY() + "," + pos.getZ() + " from network " + '[' + i + ']');
				return true;
			}
		}

		System.out.println("ERROR: No node at " + pos.getX() + "," + pos.getY() + "," + pos.getZ() + " found in network");
		return false;
	}
